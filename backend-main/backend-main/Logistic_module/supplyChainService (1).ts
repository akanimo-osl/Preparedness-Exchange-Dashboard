...
/**
 * Simulates a call to a generative AI model (like Gemini) to create a dynamic,
 * plausible outbreak scenario for a given country and hazard.
 */
...
/**
 * The core estimation engine. Calculates GROSS needs based on a scenario and the canonical database.
 */
export const calculateNeeds = (scenario: Scenario, hazard: Hazard): HazardData | null => {
    ...
    const calculatedHr = hazardData.hr.map((item) => {
        const priority = getPriority(item.priority);
...
/**
 * Calculates a NET replenishment plan by comparing gross needs to current inventory.
 */
export const calculateReplenishmentPlan = (grossNeeds: HazardData, hazard: Hazard): ReplenishmentItem[] => {
    // This is a simplified mapping for replenishment. We map the detailed items back to the shippable modules.
    ...
    if(hazard === 'cholera') {
        ...
        // Fallback for other diseases: use basic IEHK kits
    ...
    relevantCommodityIds.forEach(commodityId => {
        ...
        if (commodity) {
            ...
            // Simplified gross_need calculation for this example. A real system would map item needs back to kits.
...
/**
 * Retrieves mock shipment history for a given country.
 */
...
/**
 * Calculates inventory projections for the Planning Dashboard.
 */
export const getInventoryProjections = (item: ItemNeed, country: Country, weeks: number, demandMultiplier: number = 1) => {
    ...
    // Calculate country-specific lead time
    const base_lead_time_days = representativeModule?.avg_lead_time_days || 45; // default lead time
    const countryModifier = 1 + ((country.inform_base - 5) / 5) * 0.5; // 0-50% penalty for INFORM score > 5
    ...
    const safety_stock = weekly_demand * 2; // 2 weeks safety stock
...
/**
 * Builds a categorized replenishment report aligned to the estimator templates and current inventory.
 */
