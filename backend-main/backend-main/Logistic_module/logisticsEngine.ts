import { FormData, ForwarderKPI, RankedForwarder, ShipmentData } from '../types';

// Helper to normalize values between 0-1
function normalize(value: number, min: number, max: number): number {
    if (max === min) return 0.5;
    return (value - min) / (max - min);
}

// Calculate KPIs for forwarders using both historical data and user inputs
export function calculateForwarderKPIs(
    ...
    forwarders.forEach(f => {
        // Map forwarder name to ID format used in form keys (e.g., "Kuehne Nagel" -> "kuehne_nagel")
        const match = Object.keys(formData).find(key => 
            key.endsWith('_price') && 
            formData[key] !== undefined && // ensure it exists
            // This is a simplified matching. In a real app, we'd have a robust ID map.
            // Here we rely on the specific construction of keys in InputPanel
        ...
        if (match) {
            ...
            // Fallback for direct matches if exact naming conventions align
        ...
        // Use user input if available, otherwise use historical average
        ...
        const onTimeRate = totalShipments > 0 ? 
            forwarderData.filter(d => d.on_time).length / totalShipments : 0.85; // Default assumption of 85% reliability
...
// Rank forwarders using TOPSIS (time, cost, reliability) with weights aligned to UI copy.
export function rankForwarders(kpis: Record<string, ForwarderKPI>): RankedForwarder[] {
    ...
    // Build matrix
    const matrix = entries.map(([, data]) => criteria.map(c => data[c] ?? 0));
    ...
    // Preserve existing fields and sort
    return scored
        .map(({ name, data, score }) => ({
            name,
            ...data,
